...
## Current objective â€” DONE (v1.18 release)
...
- **New-teams-and-squad could not save â€” fixed (2 underlying bugs):**
  `teamplayerlinks` keys on `artificialkey`, which was never staged (every new
  link copied the template key â†’ duplicate-key save blocks); `NextAvailableId`
...
- `build-managed.cmd` â†’ ALL BUILDS + ENGINE TEST PASSED (engine smoke exit=0).
- App smoke EXIT=0; nav-test 23/23 OK; layout-test 575 ops 0 FAIL.
- Create-team probe â†’ FIXED verdict. Squad probe â†’ OK (0 integrity issues,
...
- **Compdata from game folder:** new "Open from Game Folderâ€¦" button in the
  Competitions â†’ Compdata tab reads TXT files from
...
- `build-managed.cmd` â†’ ALL BUILDS + ENGINE TEST PASSED (engine smoke exit=0).
- `--name-tests` â†’ 9/9 PASS (20238/20268 resolved, fallback honest).
- `--smoke` â†’ APP SMOKE OK (save round-trip verified on scratch DB).
- `--nav-test` â†’ 24/24 sections OK.
- `--layout-test` â†’ 600 layout ops OK, 0 FAIL.
- `--perf` â†’ OK (~1.4 s / 20,268 player list).

### Protected files (re-recorded hashes â€” verified 2026-08-02)
`src/database_engine.h` (887B7A35â€¦), `tests/engine_smoke.cpp` (BFF66D9Aâ€¦),
`database/fifa_ng_db.db` (A5CF1D9Dâ€¦), `database/eng_us.DB` (9E9396D3â€¦),
`src/database_engine.cpp` (92600FBEâ€¦). The previous documented values dated from before the
...
src\CM26.App\<build>\net8.0-windows\CM26_by_Rizco98.exe --name-tests "<repo>\database"
src\CM26.App\<build>\net8.0-windows\CM26_by_Rizco98.exe --smoke "<repo>\database_scratch"
src\CM26.App\<build>\net8.0-windows\CM26_by_Rizco98.exe --nav-test "<repo>\database"
src\CM26.App\<build>\net8.0-windows\CM26_by_Rizco98.exe --layout-test "<repo>\database"
```

---

## Previous task â€” 2026-07-28 (database-native player-name resolution; external TXT/XLSX dependency removed)
...
(`players.*nameid â†’ playernames â†’ eng_us.DB`). Per the honest-failure requirement: the selected
database's player names are protected by EA's second-layer text cipher (key absent â€” proven by audit),
...
- **Audit (evidence-based):** `playernames.name` = ciphered 0xC4 placeholder (0/41,190 decodable);
  `editedplayernames`/`dcplayernames` empty; `eng_us.DB` `sourcetext` uses only a 45â€“47-symbol cipher
...
- **NEW database-native, read-only stack:** `DatabasePlayerNameSource` (+ `Resolve(firstNameId,
  lastNameId, commonNameId, knownAsId)`), `LocaleStringReader` (indexed hashid/stringid â†’ text, once
  per session), `NameTextDecoder`. `PlayerNameService` refactored to a thin faÃ§ade over them.
...
### Protected files (unchanged â€” SHA-256)
`src/database_engine.h` (887B7A35â€¦), `tests/engine_smoke.cpp` (BFF66D9Aâ€¦),
`database/fifa_ng_db.db` (A5CF1D9Dâ€¦), `database/eng_us.DB` (9E9396D3â€¦).
`src/database_engine.cpp` (92600FBEâ€¦) unchanged this task; engine smoke **EXIT=0**.
...
## Previous task â€” 2026-07-28 (player-name source found + binding fixed; superseded by database-native task above)
...
### What was delivered (this task)
- **Root cause:** the team roster split the fallback string `Player {id}` on a space â†’ numeric id in
...
- **Genuine name source found (present but not loaded):** `playernames.txt` (FC26 Modern Database
  ...
  players with correct real names (Joe Cole, Iniesta, Beckham, Cristiano Ronaldo, Ã˜degaard, â€¦).
- **NEW read-only adapter** `ExternalNameSource.cs` (txtâ†’xlsxâ†’none, tolerant, never fabricates).
  Wired into `PlayerNameService` â†’ `NameResolverService` â†’ UI. New `ExternalNameRoot` setting +
...
- **Roster columns** now `Number | Display Name | Position | Overall`; the fallback is never split.
  Player Info name fields resolve IDs â†’ real names / `Unavailable`; raw IDs only in Technical tab.
...
### Protected files
`src/database_engine.h` (887B7A35â€¦), `tests/engine_smoke.cpp` (BFF66D9Aâ€¦),
`database/fifa_ng_db.db` (A5CF1D9Dâ€¦), `database/eng_us.DB` (9E9396D3â€¦) â€” **unchanged**.
`src/database_engine.cpp` is `92600FBEâ€¦` (drifted from `92600FBEâ€¦`; proven behaviour-neutral â€”
see PLAYER_NAME_BINDING_FIX_REPORT.md Â§3). Engine smoke **EXIT=0**.
...
## Previous task â€” 2026-07-26 (final release validation â€” Full Portable + Lite packages produced)
...
### TASK 1 â€” Self-contained release (Full Portable)
...
### TASK 2 â€” Framework-dependent Lite release
- `dotnet publish -r win-x64 --self-contained false` â†’ `Release\CM26_v1.0_Lite` (~6 MB, 18 files).
...
### TASK 3 â€” Clean-environment test â€” NOT TESTED (honest)
...
### TASK 4 â€” DPI validation
- 100% DPI **PASS** (native 96-DPI 1920Ã—1080 display + layout sweep, 150 ops 0 FAIL).
- 125/150/200% **NOT TESTED** â€” single 96-DPI display; higher scaling needs a Windows sign-out
  (would terminate the session). Per user direction, validated at 1920Ã—1080/100% only.
- No confirmed UI scaling defects; **no working layouts were rewritten**.

### TASK 5 â€” Release identity
...
- Settings About text updated to "v1.0.0 Â· Release Candidate 1".

### TASK 6 â€” Release documentation
...
### TASK 7 â€” Final regression (22 items)
- Full matrix executed; results in RELEASE_READINESS_REPORT.md. 1â€“17, 21, 22 **PASS**;
  18/19/20 (125/150/200% DPI) **NOT TESTED**; clean-environment bare-machine run **NOT TESTED**.
- Removed embedded absolute dev paths: `SettingsService` no longer probes `<FC26 tools>`;
  managed DLLs rebuilt with `DebugType=None` + `PathMap`; bridge linker PDB redirected to
  `C:\ProgramData` â†’ **0 dev-path hits in all exe/dll**.
...
- `src/CM26.App/AppDependencyCheck.cs` (NEW â€” Lite startup dependency check)
- `src/CM26.App/Program.cs` (invoke dependency check)
- `src/CM26.App/SettingsService.cs` (removed absolute dev path)
- `src/CM26.App/Sections/SettingsSection.cs` (About â†’ RC1)
- `src-native/CM26.EngineBridge/CM26.EngineBridge.vcxproj` (linker PDB â†’ C:\ProgramData; **engine
  sources unchanged**)
...
- `build-managed.cmd` â†’ ALL BUILDS + ENGINE TEST PASSED (bridge relinked, engine EXIT=0).
- SC publish + FDD publish â†’ both packages assembled; smoke/nav/save verified on packaged exes.

## Protected files (unchanged â€” SHA-256 verified this session)
`src/database_engine.h` (887B7A35â€¦), `src/database_engine.cpp` (92600FBEâ€¦), `tests/engine_smoke.cpp`
(BFF66D9Aâ€¦), `database/fifa_ng_db.db` (A5CF1D9Dâ€¦), `database/eng_us.DB` (9E9396D3â€¦).

## Standing limitations (unchanged, documented)
Player display names remain EA-ciphered (key absent) â†’ `Player {id}` fallback; name editing
...
.\Release\CM26_v1.0_Full_Portable\CM26_by_Rizco98.exe --smoke "<repo>\database_scratch"
```
