# Player Name Binding Fix Report â€” CM26
...
- **The protected engine was not changed** by this task. One pre-existing, behaviour-neutral
  modification to `database_engine.cpp` was investigated and dispositioned (see Â§3).
- **Fallback is honest and consistent.** Where a name cannot be resolved, the UI shows
  `Player {id}` (and `Unavailable` in the individual name fields) â€” never a raw numeric key as a name.
...
//   Surname column  â† nameParts[1]  â†’ "10264"   (the player id!)
//   First Name col  â† nameParts[0]  â†’ "Player"
```
...
either a verified real name or `Player {id}` â€” never a bare key.

---

## 3. Protected engine â€” investigation & disposition

**Finding.** `src\database_engine.cpp` SHA-256 is `92600FBEâ€¦`, but the last verified state
(`PROTECTED_ENGINE_FILES.md`, `LOCALE_DECODER_REPORT.md`, `RELEASE_READINESS_REPORT.md`,
`TASK_STATE.md`) records `92600FBEâ€¦`. The file was modified on 2026-07-27 23:16, after the verified
snapshot. No source backup exists anywhere on disk; no git repo; VSS/File History unavailable
(session not elevated).

**Disposition â€” investigated, not blindly reverted.** Because reverting was impossible (no verified
...
1. **Object-code comparison.** The pre-edit `database_engine.obj` (built 22:34 from the verified
   ...
   **identical**. The only differences are the embedded output path and nondeterministic
   compiler-generated symbols â€” i.e. formatting only, no logic change.
...
> Recommendation: re-record the `database_engine.cpp` hash as `92600FBEâ€¦` in
> `PROTECTED_ENGINE_FILES.md` once the team confirms the formatting-only edit, so future drift
> checks have an accurate baseline. (Not done here â€” that file documents the verified baseline.)

### Protected-file verification (this session)
| File | Recorded | Current | Status |
|------|----------|---------|--------|
| `src/database_engine.h` | `887B7A35â€¦` | `887B7A35â€¦` | âœ… unchanged |
| `src/database_engine.cpp` | `92600FBEâ€¦` | `92600FBEâ€¦` | âš  drifted â€” proven behaviour-neutral (see above) |
| `tests/engine_smoke.cpp` | `BFF66D9Aâ€¦` | `BFF66D9Aâ€¦` | âœ… unchanged |
| `database/fifa_ng_db.db` | `A5CF1D9Dâ€¦` | `A5CF1D9Dâ€¦` | âœ… unchanged |
| `database/eng_us.DB` | `9E9396D3â€¦` | `9E9396D3â€¦` | âœ… unchanged |
...
| File | Change |
|------|--------|
| `src/CM26.Application/Services/ExternalNameSource.cs` | **NEW.** Read-only adapter: loads `playernames.txt` (preferred) or `playernames.xlsx` into a `nameidâ†’name` map. Never writes, never fabricates; tolerant of missing/malformed files. |
...
| `src/CM26.Application/Services/NameResolverService.cs` | Accepts + passes the external root; indexes `playerid â†’ name IDs`; new `PlayerNameByPlayerId` for captain/set-piece references. |
...
| `src/CM26.App/Sections/TeamsSection.cs` | **Roster bug fix.** Columns â†’ `Number | Display Name | Position | Overall`; removed the name split; captain/set-piece fields resolve to a player name; tooltips note the player id. |
| `src/CM26.App/Sections/PlayersSection.cs` | Name fields resolve IDs â†’ real names / `Unavailable`; raw IDs only in Technical tab; tooltips explain the name-source state; "Last Name" relabelled "Surname". |
...
| `PLAYER_NAME_SOURCE_AUDIT.md`, `PLAYER_NAME_BINDING_FIX_REPORT.md` | **NEW** â€” these deliverables. |
...
| # | Test | Result |
|---|------|--------|
| 1 | Protected engine SHA-256 verification | **PASS** â€” engine.h/smoke.cpp/both data files byte-for-byte; engine.cpp drift dispositioned (Â§3) |
| 2 | Full Release x64 build (`build-managed.cmd`) | **PASS** â€” bridge + Application + App, 0 errors |
| 3 | Engine smoke test (`EngineSmokeTest.exe`) | **PASS** â€” EXIT=0 (279 tables, 360,298 rows, locale round-trip + edits verified) |
| 4 | PlayerNameService tests (`--name-tests`) | **PASS** â€” 41,189 names loaded, EXIT=0 |
...
| 7 | Team roster binding test (`Number | Display Name | Position | Overall`) | **PASS** â€” e.g. `1 | Patrick Pentz | GK | 72` |
| 8 | Formation-label binding test (nav: formations) | **PASS** |
| 9 | Captain & set-piece resolver test | **PASS** â€” e.g. `Martin Ã˜degaard`; fallback `Player {id}` |
| 10 | Search by player ID | **PASS** â€” `250` â†’ 130 matches |
| 11 | Search by real name (verified source present) | **PASS** â€” `Sangwan` â†’ 1 match |
| 12 | Fallback presentation test (source absent) | **PASS** â€” all 20,268 show `Player {id}`, no bare numeric surname, EXIT=0 |
| 13 | Scratch-copy save round-trip (`--smoke database_scratch`) | **PASS** â€” saved + reload-verified, backups created, EXIT=0 |
| 14 | Original database integrity | **PASS** â€” originals pristine; test artifacts removed |
| â€” | Nav test (17 sections) | **PASS** â€” 17/17 OK |
| â€” | Layout test (150 ops) | **PASS** â€” 0 FAIL |
| â€” | Perf (player-list build) | **PASS** â€” 20,268 players in ~1.1 s |

### Counts (live DB, source present)
- **Resolved-name count:** 20,268 / 20,268 players (100%).
- **Fallback count:** 0 (source present) Â· 20,268 (source absent, all honest `Player {id}`).
...
Settings â–¸ â€œPlayer-name source folderâ€ lets the user point at any `playernames.txt`/`.xlsx`.
...
(`<FC26 tools>\asset\fifa_ng_db\playernames.txt`, or the bundled
