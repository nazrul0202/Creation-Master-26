...
## Version 1.0.10 update — 2026-08-01
...
## Version 1.0.9 update — 2026-08-01
...
## Version 1.0.8 update — 2026-08-01
...
## Version 1.0.7 update — 2026-08-01
...
## Version 1.0.6 update — 2026-08-01
...
## Version 1.0.5 update — 2026-08-01
...
## Version 1.0.4 update — 2026-08-01
...
## Version 1.0.3 update — 2026-08-01
...
## Version 1.0.2 update — 2026-08-01
...
## Version 1.0.1 update — 2026-08-01
...
- **Still required before calling the package universally proven:** Authenticode
  signing, a real clean Windows machine test, physical 125–200% DPI tests, and
  verification against additional game patch versions.

## Official Release 1.00 update — 2026-07-30
...
- Remaining public-release gaps: clean-machine test, physical 125–200% DPI,
...
## Independent FC26 asset bridge update — 2026-07-28
...
- **NOT RETESTED:** newly published packages containing the bridge, a clean
  machine, physical 125–200% DPI, and multiple FC26 patch versions.
...
Date: 2026-07-26
Build: `CM26_by_Rizco98.exe` **v1.0.0 · Release Candidate 1** · Release | x64
Packages:
- `D:\CM 26 Final\Release\CM26_v1.0_Full_Portable` (self-contained, ~166 MB, 477 files)
- `D:\CM 26 Final\Release\CM26_v1.0_Lite` (framework-dependent, ~6 MB, 18 files)
...
|---|------|--------|------------------|
| 1 | Full Release x64 build | **PASS** | `build-managed.cmd` → EngineBridge.dll + CM26.Application.dll + CM26_by_Rizco98.dll, "ALL BUILDS + ENGINE TEST PASSED". Both packages re-published (win-x64). |
| 2 | Engine smoke test | **PASS** | RC2 `EngineSmokeTest.exe` → `state=0 tables=279`, `locale_edit_verified=20`, `main_edit_verified tables=279 rows=360298`, `structural_add_delete_verified nations=218`, **EXIT=0**. |
...
| 10 | Real asset previews | **PASS** | DXT5 miniface decodes (180×180, alpha, centre pixel read), ball/stadium/boot/glove/flag PNGs decode; `--dds-verify` RMSE **4.36** vs reference PNG (<40 = same image). |
| 11 | Missing-asset states | **PASS** | `--asset-test`: available categories resolve to real files (miniface 5/5, ball/stadium/boot 20/20, flag 30/30 of sampled); crest/minikit/logo categories correctly resolve empty → honest "No local asset" state, never a fake. |
| 12 | Save round-trip (scratch copy) | **PASS** | `nations[0].groupid 13→0` staged → 0 validation issues → saved (timestamped backup) → reloaded value **VERIFIED**. Performed only on a scratch copy. |
| 13 | Undo / redo | **PASS** | `Stage no-op edit: success=True pending=0` … `Undo OK, pending=0` through the same stack the UI uses. |
| 14 | Invalid folder | **PASS** | Garbage `fifa_ng_db.db` → clear engine error "Unsupported database folder …", graceful exit 3, no crash. |
...
| 16 | File lock | **PASS** | Load-path lock → graceful "Unsupported database folder" (no crash). Save-path lock (read-share) → `Save failed: … being used by another process.` Understandable message, originals untouched, no crash. |
| 17 | 100% DPI | **PASS** | App is PerMonitorV2 (manifest + `SetHighDpiMode`). Ran at native 96 DPI (100%) on the 1920×1080 display; GUI maximized with no errors. |
...
(150 layout operations, **0 FAIL**) across 25 sizes spanning 1180×700 → 1920×1080, plus a real
GUI launch maximized at the native 1920×1080/100% display.

| Resolution | 100% | 125% | 150% | 200% |
|------------|------|------|------|------|
| 1366×768   | PASS (via 1180×700 min-size proxy in layout sweep) | NOT TESTED | NOT TESTED | NOT TESTED |
| 1920×1080  | **PASS** (native display + GUI) | NOT TESTED | NOT TESTED | NOT TESTED |
| 2560×1440  | PASS (layout sweep ≥ this size range; no physical 2560 display) | NOT TESTED | NOT TESTED | NOT TESTED |

> **Honesty note:** this machine has a **single 1920×1080 display at 96 DPI (100%)** and the
...
> scaling cells are therefore **NOT TESTED** — they are expected-correct (PerMonitorV2) but
...
|-----------|--------|-------|
| Windows Sandbox / VM available | **No** | Windows Sandbox feature **not installed** (`WindowsSandbox.exe` absent); Hyper-V/Sandbox queries blocked — session is **not elevated**. No clean VM could be created. |
...
| Clean-folder startup (simulated) | **PASS (simulated)** | The Full Portable folder was copied/used as-is with only its bundled files + app-local VC++ CRT; headless smoke + GUI both run with **no absolute development paths** in any exe/dll (verified — 0 hits). |
...
### `CM26_v1.0_Full_Portable` (self-contained — runs with no .NET installed)
- `CM26_by_Rizco98.exe` (+ `.dll`, `.deps.json`, `.runtimeconfig.json`) — R2R, not trimmed, not single-file.
...
### `CM26_v1.0_Lite` (framework-dependent — requires .NET 8 Desktop Runtime x64)
...
| Lite release dependency documented | **YES** — `INSTALLATION.md` + README + startup dependency check |
| No absolute development paths remain | **YES** — 0 hits in all exe/dll; SettingsService dev path removed |
| All required native DLLs included | **YES** — bridge + Ijwhost + VC++ CRT in both packages |
| Clean-environment startup passes | **NOT TESTED** (no sandbox/VM; non-elevated) — simulated clean-folder PASS |
| Actual DPI scaling tested | 100% **PASS**; 125/150/200% **NOT TESTED** (single 96-DPI display, sign-out required) |
| Existing engine tests remain successful | **YES** — EXIT=0 |
| Save round-trip remains successful | **YES** — verified on scratch copy |
| Protected engine files remain unchanged | **YES** — SHA-256 verified |
| Release documentation complete | **YES** — README, INSTALLATION, KNOWN_LIMITATIONS, THIRD_PARTY_NOTICES, RELEASE_NOTES, RELEASE_READINESS_REPORT, SHA256SUMS |
| SHA-256 hashes generated | **YES** — per-package `SHA256SUMS.txt` (477 + 18 entries), verify-checked |
