...
public class DbFile
{
	...
	{
		return SaveDb(fileName, patchPlayerNames: false);
	}

	public bool SaveDb(string fileName, bool patchPlayerNames)
	{
		FileStream fileStream = new FileStream(fileName, FileMode.Create, FileAccess.ReadWrite);
		DbWriter dbWriter = new DbWriter(fileStream, m_Platform);
		SaveDb(dbWriter, patchPlayerNames);
		dbWriter.Close();
		fileStream.Close();
		ComputeAllCrc();
		return true;
	...
	{
		return SaveDb(w, patchPlayerNames: false);
	}

	public bool SaveDb(DbWriter w, bool patchPlayerNames)
	{
		if (m_ProgressBar != null)
		{
			m_ProgressBar.Maximum = m_NTables;
		...
		for (int j = 0; j < m_NTables; j++)
		{
			...
			m_Table[j].Save(w, patchPlayerNames);
		}
	...
	public void Document()
	{
		...
		for (int n = 0; n < m_NTables; n++)
		{
			...
			for (int num2 = 0; num2 < m_Table[n].TableDescriptor.NFields; num2++)
			{
				...
				streamWriter.WriteLine(tableName5 + "_" + fieldName2 + " = " + num + "; //" + fieldType);
			}
		}
		streamWriter.Close();
		fileStream.Close();
	...
	public bool Expand()
	{
		...
		flag = ExpandTableField("dcplayernames", "nameid", 15);
		bool num7 = num6 && !flag;
		RecalculateFieldOffset("dcplayernames");
	...
	private bool ExpandTableField(string tableName, string fieldName, int nBits)
	{
		...
		if (table.TableDescriptor.NBitRecords % 8 != 0)
		{
			_ = table.TableDescriptor.NBitRecords % 8;
		}
		return table.ExpandField(fieldName, nBits);
	}
