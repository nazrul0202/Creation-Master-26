...
public class HuffmannTree
{
	...
	public static HuffmannTree CreateIdentityHuffmannTree()
	{
		HuffmannTree huffmannTree = new HuffmannTree(256);
		byte b = 1;
		for (int i = 0; i <= 126; i++)
		{
			huffmannTree.m_Child[i, 0] = b++;
			huffmannTree.m_Child[i, 1] = b++;
		}
		byte b2 = 0;
		for (int j = 127; j <= 254; j++)
		{
			huffmannTree.m_Leaf[j, 0] = b2;
			b2++;
			huffmannTree.m_Leaf[j, 1] = b2;
			b2++;
		}
		huffmannTree.BuildEncodingTable();
		return huffmannTree;
	}
}
