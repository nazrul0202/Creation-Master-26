...
public class Table
{
	...
	{
		bool flag = true;
		_ = TableDescriptor.TableName == "career_firstnames";
	...
	{
		Save(w, patchPlayerNames: false);
	}

	public void Save(DbWriter w, bool patchPlayerNames)
	{
		w.Write(m_Unknown00);
		...
		for (int i = 0; i < m_NRecords; i++)
		{
			...
			else if (TableDescriptor.TableName == "career_managerpref")
			{
				num++;
			}
			else
		...
		long num3 = w.BaseStream.Position + m_NRecords * m_TableDescriptor.RecordSize;
		int num4 = 0;
		if (m_TableDescriptor.HuffmannTree != null)
		{
			if (patchPlayerNames && (m_TableDescriptor.TableName == "playernames" || m_TableDescriptor.TableName == "commentarynames"))
			{
				m_TableDescriptor.HuffmannTree = HuffmannTree.CreateIdentityHuffmannTree();
			}
			num4 = m_TableDescriptor.HuffmannTree.Size;
		}
		if (m_NRecords > 0)
		{
			num = 0;
			for (int j = 0; j < m_NValidRecords; j++)
			{
				num4 = m_Records[j].Save(w, num3, num4);
			}
			Record record = new Record(TableDescriptor);
			record.Clean();
			for (int k = m_NValidRecords; k < m_NRecords; k++)
			{
				num4 = record.Save(w, num3, num4);
			}
			if (m_TableDescriptor.HuffmannTree != null)
			{
				w.BaseStream.Position = position;
				m_CompressedStringLength = (uint)num4;
				w.Write(num4);
				w.BaseStream.Position = num3;
				m_TableDescriptor.HuffmannTree.Save(w);
			}
			num4 = FifaUtil.RoundUp(num4, 8);
			m_CrcRecordsPosition = num3 + num4;
	...
	public void SetValidRecordAndClean(int n)
	{
		_ = NValidRecords;
		for (int i = n; i < NValidRecords; i++)
		{
			m_Records[i].Clean();
		}
		NValidRecords = n;
	...
	{
		_ = TableDescriptor.TableName == "teamnationlinks";
