...
public class DxtBlock
{
	...
	private void WriteAlfaChannel(BinaryWriter bw)
	{
		for (int i = 0; i < 4; i++)
		{
			...
			num |= (ushort)(m_Alfa[1, i] & 0xF0);
			num |= (ushort)((m_Alfa[2, i] & 0xF0) << 4);
			num |= (ushort)((m_Alfa[3, i] & 0xF0) << 8);
			bw.Write(num);
		}
	}
	...
	private void WriteColorLut(BinaryWriter bw)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			b |= (byte)((m_ColorLut[1, i] & 3) << 2);
			b |= (byte)((m_ColorLut[2, i] & 3) << 4);
			b |= (byte)((m_ColorLut[3, i] & 3) << 6);
			bw.Write(b);
		}
	}
	...
	{
		int[] array = new int[8] { m_Alfa0, m_Alfa1, 0, 0, 0, 0, 0, 0 };
		if (m_Alfa0 > m_Alfa1)
		{
			for (int i = 2; i < 8; i++)
			{
	...
	{
		int[] array = new int[8] { alfa0, alfa1, 0, 0, 0, 0, 0, 0 };
		for (int i = 2; i < 8; i++)
		{
			int num = 8 - i;
			int num2 = i - 1;
