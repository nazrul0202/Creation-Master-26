...
public class FifaFat
{
	...
	public void Save()
	{
		for (int i = 0; i <= m_NBigFiles; i++)
		{
			...
			if (m_BhFiles[i] != null && m_BhFiles[i].NeedToSave)
			{
				m_BhFiles[i].Save();
			}
		}
	...
	public ArrayList FindDuplicatedFiles()
	{
		...
		{
			arrayList.Add(num + " duplicated files found.");
		}
		return arrayList;
	}
}
