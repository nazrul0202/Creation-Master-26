...
public class Player : IdObject
{
	...
	private bool m_TechDribbler;

	private bool m_ChipShot;
	...
	private static float[] c_MarketValuesGoalkeeper = new float[44]
	{
		0.01f, 0.02f, 0.03f, 0.04f, 0.05f, 0.06f, 0.07f, 0.075f, 0.08f, 0.09f,
		0.1f, 0.15f, 0.2f, 0.25f, 0.3f, 0.35f, 0.4f, 0.5f, 0.6f, 0.8f,
		1f, 1.5f, 2f, 3f, 4f, 5f, 6f, 7f, 8f, 9f,
		10f, 12f, 14f, 17f, 20f, 25f, 30f, 40f, 50f, 60f,
		80f, 100f, 120f, 150f
	};

	private static float[] c_MarketValuesDefender = new float[45]
	{
		...
		1f, 1.5f, 2f, 3f, 4f, 5f, 6f, 8f, 10f, 12f,
		15f, 20f, 25f, 30f, 35f, 40f, 50f, 60f, 70f, 80f,
		90f, 100f, 120f, 150f, 200f
	};

	private static float[] c_MarketValuesMidfielder = new float[45]
	{
		0.01f, 0.02f, 0.025f, 0.03f, 0.04f, 0.05f, 0.06f, 0.075f, 0.085f, 0.1f,
		0.075f, 0.1f, 0.15f, 0.25f, 0.35f, 0.5f, 0.65f, 0.825f, 1f, 1.25f,
		1.5f, 2f, 3f, 4f, 5f, 6f, 8f, 10f, 12f, 15f,
		20f, 25f, 30f, 35f, 40f, 50f, 60f, 65f, 75f, 90f,
		100f, 120f, 150f, 200f, 250f
	};

	private static float[] c_MarketValuesAttacker = new float[45]
	{
	...
	private static int[] s_MeanLevels = new int[7] { 52, 59, 66, 72, 77, 82, 88 };
	...
	public bool SpeedDribbler
	{
		get
		{
			return m_SpeedDribbler;
		}
		set
		{
			m_SpeedDribbler = value;
		}
	}
	...
	public bool TechDribbler
	{
		get
		{
			return m_TechDribbler;
		}
		set
		{
			m_TechDribbler = value;
		}
	}

	public bool ChipShot
	{
		get
		{
			return m_ChipShot;
	...
	public string FastRename(string newName)
	{
		...
		if (newName.Contains(value))
		{
			...
			text = newName.Substring(0, num);
			result = newName.Substring(num + 1);
			newName = null;
		}
		if (newName == null)
		{
	...
	private void StandardizeLoanEndDate()
	{
		if (!IsLoaned)
		{
			return;
		}
		int year = loandateend.Year;
		int num = loandateend.Month;
		int num2 = loandateend.Day;
		bool flag = false;
		if (num == 1 && num2 != 1)
		{
			num2 = 1;
		...
		if (flag)
		{
			loandateend = new DateTime(year, num, num2);
		}
	}

	public void ExtendLoanEndDate(int yearEnd, int monthEnd)
	{
		if (IsLoaned)
		{
			int year = loandateend.Year;
			int month = loandateend.Month;
			int day = loandateend.Day;
			if (year < yearEnd || (year == yearEnd && month <= monthEnd))
			{
				loandateend = new DateTime(yearEnd + 1, month, day);
			}
			ExtendContractAfterLoanEnd();
		}
	}

	public void ExtendContractAfterLoanEnd()
	{
		if (IsLoaned)
		{
			StandardizeLoanEndDate();
			contractvaliduntil = loandateend.Year + 1;
		}
	}

	public void RemoveFromFreeAgentIfHasClub()
	{
		bool flag = false;
		bool flag2 = false;
		Team team = null;
		for (int i = 0; i < m_PlayingForTeams.Count; i++)
		{
			Team team2 = (Team)m_PlayingForTeams[i];
			if (team2.IsClub())
			{
				flag = true;
			}
			else if (team2.Id == 111592)
			{
				flag2 = true;
				team = team2;
			}
		...
		if (flag2 && flag)
		{
			team.RemoveTeamPlayer(this);
		}
	}

	public void AddToFreeAgentIfWithoutClub()
	{
		Team team = (Team)FifaEnvironment.Teams.SearchId(111592);
		for (int i = 0; i < m_PlayingForTeams.Count; i++)
		{
			Team team2 = (Team)m_PlayingForTeams[i];
			if (team2.IsClub() || team2.Id == 111592)
			{
				return;
			}
		}
	...
	public void UpdateNamesAndCommentary()
	{
		...
		if (!PlayerNames.TryGetValue(m_lastnameid, out m_lastname, out var _, isUsed: true))
		{
			m_lastname = string.Empty;
		}
		if (!PlayerNames.TryGetValue(m_commonnameid, out m_commonname, out var _, isUsed: true))
		{
			m_commonname = string.Empty;
		}
	...
	public long PackAllTraits()
	{
		return 0L + (long)(m_Inflexible ? 1 : 0) + (m_Longthrows ? 2 : 0) + (m_PowerfulFreeKicks ? 4 : 0) + (m_Diver ? 8 : 0) + (m_InjuryProne ? 16 : 0) + (m_InjuryFree ? 32 : 0) + (m_AvoidsWeakFoot ? 64 : 0) + (m_Divesintotackles ? 128 : 0) + (m_BeatDefensiveLine ? 256 : 0) + (m_Selfish ? 512 : 0) + (m_Leadership ? 1024 : 0) + (m_ArguesWithOfficials ? 2048 : 0) + (m_Earlycrosser ? 4096 : 0) + (m_FinesseShot ? 8192 : 0) + (m_Flair ? 16384 : 0) + (m_LongPasser ? 32768 : 0) + (m_LongShotTaker ? 65536 : 0) + (m_SpeedDribbler ? 131072 : 0) + (m_Playmaker ? 262144 : 0) + (m_Pushesupforcorners ? 524288 : 0) + (m_Puncher ? 1048576 : 0) + (m_GkLongThrower ? 2097152 : 0) + (m_PowerHeader ? 4194304 : 0) + (m_GkOneOnOne ? 8388608 : 0) + (m_GiantThrow ? 16777216 : 0) + (m_OutsideFootShot ? 33554432 : 0) + (m_CrowdFavorite ? 67108864 : 0) + (m_SwervePasser ? 134217728 : 0) + (m_SecondWind ? 268435456 : 0) + (m_AcrobaticClearance ? 536870912 : 0) + (m_FancyFeet ? 4294967296L : 0) + (m_FancyPasses ? 8589934592L : 0) + (m_FancyFlicks ? 17179869184L : 0) + (m_StutterPenalty ? 34359738368L : 0) + (m_ChipperPenalty ? 68719476736L : 0) + (m_BycicleKick ? 137438953472L : 0) + (m_DivingHeader ? 274877906944L : 0) + (m_DrivenPass ? 549755813888L : 0) + (m_GkFlatKick ? 1099511627776L : 0) + (m_HighClubIdentification ? 512 : 0) + (m_TeamPlayer ? 1024 : 0) + (m_ChipShot ? 2048 : 0) + (m_TechDribbler ? 4096 : 0);
	}
	...
	public void Load(Record r)
	{
		...
		m_SpeedDribbler = (((andCheckIntField & 0x20000) != 0) ? true : false);
		...
		m_ChipShot = (((andCheckIntField2 & 0x800) != 0) ? true : false);
		m_TechDribbler = (((andCheckIntField2 & 0x1000) != 0) ? true : false);
		m_assetid = base.Id;
	...
	public void UpdateFromOnlineRecord19(Record r, TableDescriptor td)
	{
	...
	public void UpdateFromRecord20(Record r, TableDescriptor td)
	{
		DateTime dateTime = FifaUtil.ConvertToDate(r.GetAndCheckIntField(td.GetFieldIndex("birthdate")));
		if (dateTime.Year < 1900)
		{
		...
		if (m_headtypecode >= 533 && m_headtypecode <= 581)
		{
			m_headtypecode = m_Randomizer.Next(500, 533);
		}
		_ = m_headtypecode;
	...
	public void FillFromPlayerloans(Table t)
	{
		...
		for (int i = 0; i < t.NRecords; i++)
		{
			...
			if (record.GetAndCheckIntField(FI.playerloans_playerid) == base.Id)
			{
				m_teamidloanedfrom = record.GetAndCheckIntField(FI.playerloans_teamidloanedfrom);
				m_loandateend = FifaUtil.ConvertToDate(record.GetAndCheckIntField(FI.playerloans_loandateend));
				m_IsLoaned = true;
				break;
			}
		}
	...
	public void FillFromPreviousTeam(Table t)
	{
		...
		{
			Record record = t.Records[i];
			if (record.GetAndCheckIntField(FI.previousteam_playerid) == base.Id)
			{
				m_previousteamid = record.GetAndCheckIntField(FI.previousteam_previousteamid);
				break;
			}
		}
	}
	...
	public override IdObject Clone(int playerid)
	{
		...
		player.m_lastname = "Player_" + player.Id;
	...
	public Player Copy(Player clone)
	{
		...
		clone.m_SpeedDribbler = m_SpeedDribbler;
		...
		clone.m_ChipShot = m_ChipShot;
		clone.m_TechDribbler = m_TechDribbler;
	...
	{
		if (m_birthdate.Month == 2 && m_birthdate.Day == 29 && base.Id > 230000)
		{
			return true;
		}
		return false;
	...
	{
		return "data/sceneassets/faces/face_" + id + "_0_0_0_0_0_0_0_0_textures.rx3";
	}

	public string GenericFaceTextureFileName()
	{
		return "data/sceneassets/genheadtex/skin_" + m_skintonecode + "_" + m_skintypecode + ".rx3";
	}

	public string GenericSkinTextureFileName()
	{
		string text = null;
		if (m_skintonecode == 1 || m_skintonecode == 3)
		{
			return "data/sceneassets/genheadtex/skin_" + (m_skintonecode + 1) + "_" + (m_skintypecode + 3) + ".rx3";
		}
		return "data/sceneassets/genheadtex/skin_" + m_skintonecode + "_" + m_skintypecode + ".rx3";
	}
	...
	{
		string text = null;
		if (skintone == 1 || skintone == 3)
		{
			return "data/sceneassets/genheadtex/skin_" + (skintone + 1) + "_" + (skintype + 3) + ".rx3";
		}
		return "data/sceneassets/genheadtex/skin_" + skintone + "_" + skintype + ".rx3";
	}
	...
	{
		return "data/sceneassets/genheadtex/brow_" + m_skintonecode + "_" + m_sideburnscode + "_" + m_facialhaircolorcode + ".rx3";
	}
	...
	{
		return "data/sceneassets/heads/head_" + id + "_0.rx3";
	}
	...
	{
		return "data/sceneassets/hair/hair_" + hairtypecode + "_1_0.rx3";
	}

	public static string GenericHairLodModelFileName(int hairtypecode)
	{
		return "data/sceneassets/hairlod/hairlod_" + hairtypecode + "_1_0.rx3";
	}
	...
	{
		return "data/sceneassets/hairlod/hairlod_" + id + "_0_0.rx3";
	}
	...
	{
		return "data/sceneassets/hair/hair_" + id + "_0_0.rx3";
	}
	...
	{
		return "data/sceneassets/heads/eyes_" + id + "_0_textures.rx3";
	}
	...
	{
		return "data/sceneassets/body/skin_" + m_skintonecode + "_" + (Female ? 1 : 0) + "_textures.rx3";
	}

	public string TattoTextureFileName()
	{
		return "data/sceneassets/tattoo/tattoo_" + base.Id + "_0.rx3";
	}

	public string RevModSkinTextureFileName()
	{
		return "data/sceneassets/body/playerskin_" + base.Id + "_textures.rx3";
	}
	...
	public static string PhotoTemplateFileName()
	{
		return "data/ui/imgassets/heads/p#.dds";
	}
	...
	{
		return "data/ui/imgassets/heads/p" + base.Id + ".dds";
	}

	public static string SpecificPhotoDdsFileName(int id)
	{
		return "data/ui/imgassets/heads/p" + id + ".dds";
	}
	...
	public void RemoveCurrentConflictingTeam(Team newTeam)
	{
		if (m_PlayingForTeams == null)
		{
			return;
		...
		foreach (Team playingForTeam in m_PlayingForTeams)
		{
			if (playingForTeam != newTeam)
			{
				if (newTeam.IsClub() && playingForTeam.IsClub())
				{
					playingForTeam.RemoveTeamPlayer(this);
					break;
				}
				if (newTeam.IsNationalTeam() && playingForTeam.IsNationalTeam())
				{
					playingForTeam.RemoveTeamPlayer(this);
					break;
				}
	...
	public int EstimateSkills(float marketValue, int age, ERole role)
	{
		...
		case ERole.Left_Midfielder:
			array = c_MarketValuesMidfielder;
			break;
		...
		else if (age >= 30 && role != ERole.Goalkeeper)
		{
			if (age <= 31)
			{
				num += 2;
	...
	public int EstimatePotential(int age)
	{
		int num = GetAverageRoleAttribute();
		if (age <= 18)
		{
			num = num + 4 + 5;
		}
		...
		if (age <= 24)
		{
			m_potential = num + m_Randomizer.Next(0, 6);
		}
		else if (age <= 27)
		{
			m_potential = num + m_Randomizer.Next(2, 5);
		}
		else if (age <= 29)
		{
			m_potential = num + m_Randomizer.Next(0, 3);
		}
		else
		{
			m_potential = num;
	...
	{
		Bitmap bitmap = FifaEnvironment.GetBmpFromRx3(GenericSkinTextureFileName());
		if (bitmap == null)
		{
			return null;
		}
	...
	{
		return new Rx3Signatures(175072, 24, new string[1] { "body_" + m_skintonecode + "_cm.Raster" });
	}
	...
	{
		return new Rx3Signatures(11200, 24, new string[1] { "eyes_" + base.Id + "_0_cm.Raster" });
	}
	...
	{
		return "data/sceneassets/hair/hair_" + id + "_0_textures.rx3";
	}
	...
	public Bitmap GetGenericHairColorTexture()
	{
		...
		if (bmpsFromRx != null)
		{
			if (m_haircolorcode >= s_GenericColors.Length)
			{
				m_haircolorcode = 0;
			}
	...
	private string ConvertHeadModel(string rx3FileName)
	{
		...
		for (int i = 0; i < rx3File.Rx3Header.NFiles; i++)
		{
	...
	public bool UpdateHairModel(Model3D newModel4, Model3D newModel5)
	{
		if (!HasSpecificHeadModel)
		{
			FifaEnvironment.UserMessages.ShowMessage(5062);
		...
		if (newModel4 != null && m_HairModelFile.Rx3VertexArrays.Length >= 1 && m_HairModelFile.Rx3VertexArrays[0] != null)
		{
			m_HairModelFile.Rx3VertexArrays[0].SetVertex(newModel4.Vertex);
			m_HairModelFile.Rx3IndexArrays[0].SetIndexStream(newModel4.Index);
		}
		if (newModel5 != null && m_HairModelFile.Rx3VertexArrays.Length >= 2 && m_HairModelFile.Rx3VertexArrays[1] != null)
		{
			m_HairModelFile.Rx3VertexArrays[1].SetVertex(newModel5.Vertex);
			m_HairModelFile.Rx3IndexArrays[1].SetIndexStream(newModel5.Index);
		}
		m_HairModelFile.Save(FifaEnvironment.GameDir + SpecificHairModelFileName(), saveBitmaps: false, saveVertex: true);
		return true;
	}

	public bool UpdateHairVertex(PositionNormalTextured[] newVertex4, PositionNormalTextured[] newVertex5, bool invertClockwise)
	{
		if (!HasSpecificHeadModel)
		{
			FifaEnvironment.UserMessages.ShowMessage(5062);
		...
		if (newVertex4 != null)
		{
			if (m_HairModelFile.Rx3VertexArrays.Length >= 1 && m_HairModelFile.Rx3VertexArrays[0] != null)
			{
				m_HairModelFile.Rx3VertexArrays[0].SetVertex(newVertex4);
			}
			if (invertClockwise)
			{
				ushort[] indexStream = m_HairModelFile.Rx3IndexArrays[0].IndexStream;
				int nFaces = m_HairModelFile.Rx3IndexArrays[0].nFaces;
				for (int i = 0; i < nFaces; i++)
				{
					ushort num = indexStream[i * 3 + 1];
					indexStream[i * 3 + 1] = indexStream[i * 3 + 2];
					indexStream[i * 3 + 2] = num;
				}
			}
		}
		if (newVertex5 != null)
		{
			if (m_HairModelFile.Rx3VertexArrays.Length >= 2 && m_HairModelFile.Rx3VertexArrays[1] != null)
			{
				m_HairModelFile.Rx3VertexArrays[1].SetVertex(newVertex5);
			}
			if (invertClockwise)
			{
				ushort[] indexStream2 = m_HairModelFile.Rx3IndexArrays[1].IndexStream;
				int nFaces2 = m_HairModelFile.Rx3IndexArrays[1].nFaces;
				for (int j = 0; j < nFaces2; j++)
				{
					ushort num2 = indexStream2[j * 3 + 1];
					indexStream2[j * 3 + 1] = indexStream2[j * 3 + 2];
					indexStream2[j * 3 + 2] = num2;
				}
			}
		}
		m_HairModelFile.Save(FifaEnvironment.GameDir + SpecificHairModelFileName(), saveBitmaps: false, saveVertex: true);
	...
	public bool UpdateHairVertex(PositionNormalTextured[] newVertex4, PositionNormalTextured[] newVertex5)
	{
		return UpdateHairVertex(newVertex4, newVertex5, invertClockwise: false);
	}

	public bool UpdateHeadVertex(PositionNormalTextured[] newVertexHead, PositionNormalTextured[] newVertexEyes)
	{
		if (!HasSpecificHeadModel)
		{
			FifaEnvironment.UserMessages.ShowMessage(5062);
	...
	public bool SetHairTextures(Bitmap colorBitmap, Bitmap alfaBitmap)
	{
		...
		return SetHairTextures(new Bitmap[2] { alfaBitmap, colorBitmap });
	}

	public void SavePlayer(Record r)
	{
		SavePlayer(r, forceNameIds: false);
	}

	public void SavePlayer(Record r, bool forceNameIds)
	{
		if (!forceNameIds)
		{
			string name = null;
			...
			if (name != m_lastname)
			{
				m_lastnameid = s_PlayerNames.GetKey(m_lastname);
			}
			name = null;
		...
		num2 |= (m_ChipShot ? 2048 : 0);
		num2 |= (m_TechDribbler ? 4096 : 0);
	...
	public void RandomizeAttributes(int level)
	{
		...
		int num2 = m_preferredposition1;
		int val = c_Attributes[num2, 0] * num / 100;
		val = FifaUtil.Limit(val, 11, 90);
		int val2 = c_Attributes[num2, 1] * num / 100;
		val2 = FifaUtil.Limit(val2, 11, 90);
		int val3 = c_Attributes[num2, 2] * num / 100;
		val3 = FifaUtil.Limit(val3, 11, 90);
		int val4 = c_Attributes[num2, 3] * num / 100;
		...
		int num3 = 20 + level * 8 + (m_height - 175) * 2;
		if (num3 > 90)
		{
			num3 = 90;
		}
		...
		m_headingaccuracy = m_Randomizer.Next(num3 - 10, num3 + 10);
		...
		m_freekickaccuracy = m_Randomizer.Next(num4 - 10, num4 + 10);
		m_reactions = m_Randomizer.Next(num4 - 10, num4 + 10);
		m_agility = m_Randomizer.Next(num4 - 10, num4 + 10);
		m_balance = m_Randomizer.Next(num4 - 10, num4 + 10);
		m_jumping = m_Randomizer.Next(num4 - 10, num4 + 10);
		m_overallrating = GetAverageRoleAttribute();
		int num5 = 2000 + FifaEnvironment.Year - m_birthdate.Year;
		if (num5 > 30)
		{
		...
		else
		{
			...
			minValue = 1 + (25 - num5) / 1;
			maxValue = 5 + (25 - num5) * 4;
		...
		else
		{
			...
			if (m_dribbling > 75)
			{
				m_SpeedDribbler = m_Randomizer.Next(0, 100) <= m_dribbling - 70;
			}
			if (m_curve > 75)
			{
		...
		int num6 = level - 1;
		m_internationalrep = ((num6 < 1) ? 1 : num6);
		m_overallrating = GetAverageRoleAttribute();
	}
	...
	{
		_ = new int[10] { 1, 0, 1, 0, 1, 3, 2, 4, 3, 3 };
		bool flag = false;
		bool flag2 = false;
	...
	{
		int[] array = new int[10] { 1, 0, 1, 0, 1, 3, 2, 4, 3, 3 };
	...
	public static int RandomizeNumber(int minInclusive, int maxExclusive)
	{
		return m_Randomizer.Next(minInclusive, maxExclusive);
	}
	...
	public int ComputeMeanAttributes(int type)
	{
		return type switch
		{
			0 => (m_gkpositioning + m_gkdiving + m_gkreflexes + m_gkhandling + m_gkkicking) / 5, 
			1 => (m_aggression + m_standingtackle + m_slidingtackle + m_marking + m_interceptions) / 5, 
	...
	{
		int num = 0;
		switch ((ERole)m_preferredposition1)
		{
		case ERole.Goalkeeper:
		...
		case ERole.Left_Central_Midfielder:
			num = (m_stamina * 6 + m_reactions * 8 + m_interceptions * 5 + m_positioning * 6 + m_vision * 13 + m_ballcontrol * 14 + m_dribbling * 7 + m_finishing * 2 + m_longpassing * 13 + m_shortpassing * 17 + m_longshots * 4 + m_standingtackle * 5 + 50) / 100;
			if (num > 99)
			{
				num = 99;
			}
	...
	{
		int num = m_preferredposition1;
		return GetAverageRoleAttribute() * c_RolesMap[num, (int)requestedRole] / 100;
	}
	...
	public ERole ChooseRole(ERole[] availableRoles, int nRoles)
	{
		...
		if ((m_preferredposition1 == 0 && eRole != ERole.Goalkeeper) || (m_preferredposition1 != 0 && eRole == ERole.Goalkeeper))
		{
	...
	public void ChangeRoleSkills(int delta)
	{
		switch ((ERole)m_preferredposition1)
		{
		case ERole.Goalkeeper:
			...
			m_gkpositioning += delta;
			if (m_gkpositioning > 99)
			{
				m_gkpositioning = 99;
			}
			m_gkkicking += delta;
			...
			break;
		case ERole.Sweeper:
			m_interceptions += delta;
			if (m_interceptions > 99)
			{
				m_interceptions = 99;
			}
			m_standingtackle += delta;
			if (m_standingtackle > 99)
			{
				m_standingtackle = 99;
			}
			m_marking += delta;
			...
			m_slidingtackle += delta;
			if (m_slidingtackle > 99)
			{
				m_slidingtackle = 99;
			}
			m_headingaccuracy += delta;
			if (m_headingaccuracy > 99)
			{
				m_headingaccuracy = 99;
			}
			m_ballcontrol += delta;
			if (m_ballcontrol > 99)
			{
			...
			m_shortpassing += delta;
			if (m_shortpassing > 99)
			{
				m_shortpassing = 99;
			}
			m_reactions += delta;
	...
	public void UpdatePlayername19(Table dcplayernamesTable, Table originalPlayernamesTable)
	{
	}

	public void UpdatePlayername(Table dcplayernamesTable, Table originalPlayernamesTable)
	{
		int rangeLow = dcplayernamesTable.TableDescriptor.GetFieldDescriptor("nameid").RangeLow;
		if (firstnameid >= rangeLow)
		{
		...
		if (playerjerseynameid >= rangeLow)
		{
