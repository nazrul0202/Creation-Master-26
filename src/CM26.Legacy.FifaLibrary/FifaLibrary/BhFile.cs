...
public class BhFile
{
	...
	public bool NeedToSave => m_NeedToSave;
	...
	public bool Load(FifaBigFile bigFile, bool hideExternalFiles)
	{
		...
		if (FifaEnvironment.Year == 14)
		{
			...
			text = ((num < 0) ? (Path.GetDirectoryName(bigFile.PhysicalName) + "\\") : bigFile.PhysicalName.Substring(0, num + 6));
		}
		else
		{
			int num = bigFile.PhysicalName.IndexOf("\\FIFA 16\\", 0, StringComparison.InvariantCultureIgnoreCase);
			text = ((num < 0) ? (Path.GetDirectoryName(bigFile.PhysicalName) + "\\") : bigFile.PhysicalName.Substring(0, num + 9));
		}
