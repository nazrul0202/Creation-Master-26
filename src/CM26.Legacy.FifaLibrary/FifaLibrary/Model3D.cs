...
public class Model3D
{
	...
	private PositionNormalTextured[] m_Vertex;
	...
	public PositionNormalTextured[] Vertex => m_Vertex;

	public ushort[] Index => m_Index;
	...
	private void ChangeIndexClockwise()
	{
		for (int i = 0; i < m_NFaces; i++)
		{
			ushort num = m_Index[i * 3 + 1];
			m_Index[i * 3 + 1] = m_Index[i * 3 + 2];
			m_Index[i * 3 + 2] = num;
		}
	}
	...
	private void SetIndexArray(Rx3IndexArray indexArray)
	{
		...
		m_Index = new ushort[m_NFaces * 3];
		int num = 0;
		int num2 = ((Rx3IndexArray.TriangleListType == Rx3IndexArray.ETriangleListType.InvertOdd) ? 1 : 0);
		for (int j = 0; j < m_NIndex - 2; j++)
		{
			ushort num3 = m_IndexStream[j];
			ushort num4 = m_IndexStream[j + 1];
			ushort num5 = m_IndexStream[j + 2];
	...
	private void SetVertexArray(Rx3VertexArray vertexArray)
	{
		...
		m_Vertex = new PositionNormalTextured[m_NVertex];
		_ = vertexArray.VertexSize;
		for (int i = 0; i < m_NVertex; i++)
		{
			m_Vertex[i] = default(PositionNormalTextured);
	...
	public Model3D Clone()
	{
		...
		obj.m_Index = (ushort[])m_Index.Clone();
		obj.m_IndexStream = (ushort[])m_IndexStream.Clone();
		obj.m_Vertex = (PositionNormalTextured[])m_Vertex.Clone();
		return obj;
	}
	...
	private void ComputeNormals()
	{
		...
		for (int i = 0; i < m_NFaces; i++)
		{
			...
			Vector3 right2 = Vector3.Normalize(Vector3.Cross(vector2: Vector3.Subtract(left, right), vector1: Vector3.Subtract(left2, right)));
			array[num] = Vector3.Add(array[num], right2);
			array[num2] = Vector3.Add(array[num2], right2);
			array[num3] = Vector3.Add(array[num3], right2);
		}
		for (int j = 0; j < m_NVertex; j++)
		{
			Vector3 vector = Vector3.Normalize(array[j]);
			vector.Length();
	...
	public void Mirror()
	{
		for (int i = 0; i < m_NVertex; i++)
		{
			m_Vertex[i].X = 0f - m_Vertex[i].X;
			m_Vertex[i].Nx = 0f - m_Vertex[i].Nx;
			m_Vertex[i].Ny = m_Vertex[i].Ny;
			m_Vertex[i].Nz = m_Vertex[i].Nz;
		}
		for (int j = 0; j < m_NFaces; j++)
		{
			ushort num = m_Index[j * 3 + 1];
			m_Index[j * 3 + 1] = m_Index[j * 3 + 2];
			m_Index[j * 3 + 2] = num;
		}
		ComputeNormals();
	}
	...
	public void RemoveCiglia()
	{
		for (int i = 0; i < m_NVertex; i++)
		{
			if (m_Vertex[i].Z > 8f && m_Vertex[i].Y < 174f && m_Vertex[i].Y > 170f)
			{
				m_Vertex[i].X = 0f;
				m_Vertex[i].Y = 172f;
				m_Vertex[i].Z = 8f;
	...
	{
		for (int i = 1; i < m_NVertex; i++)
		{
			m_Vertex[i].X += m_Vertex[i - 1].X;
			m_Vertex[i].Y += m_Vertex[i - 1].Y;
			m_Vertex[i].Z += m_Vertex[i - 1].Z;
		}
	}
