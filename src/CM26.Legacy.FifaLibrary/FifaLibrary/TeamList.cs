...
{
	private static int s_MinId = 130020;

	private static int s_MaxId = 131071;

	private static char[] s_Separators = new char[3] { ' ', '.', '-' };

	public static int TeamIdStart
	{
		get
		{
			return s_MinId;
	...
	public void FillFromTeamPlayerLinks(Table t)
	{
		...
		int fieldIndex = t.TableDescriptor.GetFieldIndex("teamid");
		int fieldIndex2 = t.TableDescriptor.GetFieldIndex("playerid");
		for (int i = 0; i < t.NValidRecords; i++)
		{
			Record record = t.Records[i];
	...
	{
		Table table = eaDbFile.GetTable("teams");
		Table table2 = eaDbFile.GetTable("teamstadiumlinks");
		eaDbFile.GetTable("teamnationlinks");
		Table table3 = eaDbFile.GetTable("stadiumassignments");
		Table table4 = eaDbFile.GetTable("rowteamnationlinks");
		Table table5 = eaDbFile.GetTable("teamplayerlinks");
		Table table6 = eaDbFile.GetTable("formations");
		Table table7 = eaDbFile.GetTable("default_teamsheets");
		int num = 0;
		int num2 = 0;
		int num3 = 0;
		int fieldIndex = table6.TableDescriptor.GetFieldIndex("teamid");
		int num4 = 0;
		for (int i = 0; i < table6.NValidRecords; i++)
		{
			if (table6.Records[i].IntField[fieldIndex] != -1)
			{
				num4 = i;
				break;
			}
		...
		try
		{
			while (enumerator.MoveNext())
			{
				...
				{
					Record r2 = table5.Records[num2];
					num2++;
					item.Save(r2, num3);
					num3++;
				}
				if (num < table7.Records.Length)
				{
					Record r3 = table7.Records[num];
					team.SaveDefaultTeamsheets(r3);
				}
				Record r4 = table2.Records[num];
				team.SaveTeamStadiumLinks(r4);
				Record r5 = table3.Records[num];
				team.SaveStadiumAssignment(r5);
				if (num < table4.Records.Length)
				{
					Record r6 = table4.Records[num];
					team.SaveRowTeamNationLinks(r6);
				}
				if (team.Formation != null)
				{
					Record r7 = table6.Records[num5];
					team.Formation.Save(r7);
					num5++;
				}
				num++;
			}
		...
		table.SetValidRecordAndClean(num);
		table7.SetValidRecordAndClean(num);
		table2.SetValidRecordAndClean(num);
		table3.SetValidRecordAndClean(num);
		table4.SetValidRecordAndClean(num);
		table5.SetValidRecordAndClean(num2);
		table6.SetValidRecordAndClean(num5);
	}
	...
	public override IdArrayList Filter(IdObject filter)
	{
		...
		if (filter.GetType().Name == "NoLeagueClub")
		{
			for (int k = 0; k < Count; k++)
			{
				Team team3 = (Team)this[k];
		...
		if (filter.GetType().Name == "MissedKitClub")
		{
			for (int l = 0; l < Count; l++)
			{
				Team team4 = (Team)this[l];
				if (team4.GetKit(0) == null || team4.GetKit(1) == null)
				{
					teamList.Add(team4);
				}
			}
