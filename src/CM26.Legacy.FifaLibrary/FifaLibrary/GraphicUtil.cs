...
public class GraphicUtil
{
	...
	public static Bitmap CanvasSizeBitmap(Bitmap sourceBitmap, Rectangle section)
	{
		if (sourceBitmap == null)
		{
			return null;
		}
		Bitmap bitmap = new Bitmap(section.Width, section.Height, PixelFormat.Format32bppArgb);
		Graphics graphics = Graphics.FromImage(bitmap);
	...
	public static bool ColorizeGrayScale(Bitmap sourceBitmap, Color color1)
	{
		if (sourceBitmap == null)
		{
			return false;
		...
		for (int i = 0; i < num; i++)
		{
			...
			int num3 = array[i * 4 + 1];
			int num4 = array[i * 4 + 2];
			int num5 = color1.R * num4 / 256;
			int num6 = color1.G * num3 / 256;
			int num7 = color1.B * num2 / 256;
			if (num5 > 255)
			{
				num5 = 255;
			}
	...
	public static Bitmap MultiplyBitmap(Bitmap sourceBitmap, Bitmap multBitmap)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			int num3 = (int)((array2[i] & 0xFF000000u) >> 24);
	...
	public static Bitmap MultiplyColorToBitmap(Bitmap sourceBitmap, Color color, int divisor, bool preserveAlfa)
	{
		...
		for (int j = 0; j < num; j++)
		{
			Color color3 = Color.FromArgb(array[j]);
			int num5 = color3.A;
			int num6 = color.R * color3.R / (int)num3;
			int num7 = color.G * color3.G / (int)num3;
			int num8 = color.B * color3.B / (int)num3;
			if (num6 > 255)
			{
				if (num6 > num4)
				{
			...
			array[j] = (((((num5 << 8) | num6) << 8) | num7) << 8) | num8;
		}
		Bitmap bitmap = new Bitmap(sourceBitmap.Width, sourceBitmap.Height, PixelFormat.Format32bppArgb);
		rect = new Rectangle(0, 0, sourceBitmap.Width, sourceBitmap.Height);
		BitmapData bitmapData2 = bitmap.LockBits(rect, ImageLockMode.WriteOnly, bitmap.PixelFormat);
	...
	public static void ColorizeWhiteSponsor(Bitmap srcBitmap, Color color)
	{
		if (srcBitmap == null)
		{
			return;
	...
	public static void DrawOver(Bitmap belowBitmap, Bitmap overBitmap, int x, int y)
	{
		Graphics graphics = Graphics.FromImage(belowBitmap);
		graphics.DrawImage(overBitmap, x, y, overBitmap.Width, overBitmap.Height);
		graphics.Dispose();
	}

	public static void DrawOver(Bitmap belowBitmap, Bitmap overBitmap, Rectangle destRectangle)
	{
		Graphics graphics = Graphics.FromImage(belowBitmap);
		graphics.DrawImage(overBitmap, destRectangle);
		graphics.Dispose();
	}
		...
		if (pixel.A == byte.MaxValue)
		{
			...
			for (int i = 0; i < bitmap.Width; i++)
			{
				for (int j = 0; j < bitmap.Height; j++)
				{
					...
					if (Math.Abs(pixel2.R - pixel.R) <= 15 && Math.Abs(pixel2.G - pixel.G) <= 15 && Math.Abs(pixel2.B - pixel.B) <= 15)
					{
	...
	public static Bitmap CreateReferenceBitmap(Bitmap sourceBitmap, Color c1, Color c2, Color c3)
	{
		...
		for (int j = 0; j < num; j++)
		{
			...
			int num5 = rgb[2];
			int num2 = 255;
			array[j] = (((((num2 << 8) | num3) << 8) | num4) << 8) | num5;
		}
		Bitmap bitmap = new Bitmap(sourceBitmap.Width, sourceBitmap.Height, PixelFormat.Format32bppArgb);
		rect = new Rectangle(0, 0, sourceBitmap.Width, sourceBitmap.Height);
		BitmapData bitmapData2 = bitmap.LockBits(rect, ImageLockMode.WriteOnly, bitmap.PixelFormat);
	...
	public static Bitmap CreateReferenceBitmapPreservingAlpha(Bitmap sourceBitmap, Color c1, Color c2, Color c3)
	{
		...
		{
			Color color = Color.FromArgb(array[i]);
			array3[i] = (byte)((array[i] & 0xFF000000u) >> 24);
			int num3;
			int num4;
			int num5;
			int num2 = (num3 = (num4 = (num5 = 0)));
			...
			if (num2 == 255)
			{
				array[i] = (((((array3[i] << 8) | num3) << 8) | num4) << 8) | num5;
				continue;
			}
			num3 = color.R;
			num4 = color.G;
		...
		for (int j = 0; j < num; j++)
		{
			...
			int num5 = rgb[2];
			int num2 = 255;
			array[j] = (((((array3[j] << 8) | num3) << 8) | num4) << 8) | num5;
		}
		Bitmap bitmap = new Bitmap(sourceBitmap.Width, sourceBitmap.Height, PixelFormat.Format32bppArgb);
		rect = new Rectangle(0, 0, sourceBitmap.Width, sourceBitmap.Height);
		BitmapData bitmapData2 = bitmap.LockBits(rect, ImageLockMode.WriteOnly, bitmap.PixelFormat);
	...
	public static int[] ComputeRGBPercentage(Bitmap bitmap, Rectangle rectangle, int minLevel)
	{
		int[] array = new int[3];
		for (int i = rectangle.X; i < rectangle.X + rectangle.Width; i++)
		{
		...
		int num = rectangle.Width * rectangle.Height;
		array[0] = array[0] * 100 / num;
		array[1] = array[1] * 100 / num;
		array[2] = array[2] * 100 / num;
		return array;
	}
