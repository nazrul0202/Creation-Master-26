...
public class StadiumList : IdArrayList
{
	...
	public void FillFromAudiostadium(Table t)
	{
		int fieldIndex = t.TableDescriptor.GetFieldIndex("stadiumid");
		for (int i = 0; i < t.NValidRecords; i++)
		{
			Record record = t.Records[i];
			int id = record.IntField[fieldIndex];
			((Stadium)SearchId(id))?.FillFromAudiostadium(record);
		}
	}
	...
	public void Save(DbFile fifaDbFile)
	{
		...
		Table table2 = fifaDbFile.Table[TI.audiostadium];
		table2.ResizeRecords(Count);
		int num = 0;
		IEnumerator enumerator = GetEnumerator();
		try
		{
