...
public class Task
{
	...
	public void LinkGroup(Group ownerGroup)
	{
		...
		else if (m_Action == "FillWithTeam")
		{
			...
			m_Trophy2 = null;
			m_League = null;
			Team team = (Team)FifaEnvironment.Teams.SearchId(m_Parameter2);
			m_Team = team;
		}
	...
	public override string ToString()
	{
		...
		if (m_Action == "UpdateTable")
		{
			if (m_Group != null)
			{
				text = "Team n." + m_Parameter2 + " of " + m_Group.ParentStage.ToString() + " - " + m_Group.ToString();
			}
		}
		else if (m_Action == "ClearLeagueStats")
		{
		...
		else
		{
			...
			case EQualifyingRule.FillFromCompTable:
				if (m_Trophy1 != null)
				{
					text = ((m_Parameter2 != 1) ? ("Get best " + m_Parameter2 + " teams of " + m_Trophy1.ToString()) : ("Get winner of " + m_Trophy1.ToString()));
			...
			case EQualifyingRule.FillFromLeagueMaxFromCountry:
				if (m_League != null)
				{
					text = "Get up to " + m_Parameter2 + " team(s) from league: " + m_League.ToString() + " (max " + m_Parameter3 + ")";
					m_Parameter1 = m_League.Id;
				}
				break;
			case EQualifyingRule.FillFromSpecialTeams:
				text = "Get " + m_Parameter1 + " team(s) from Special Teams Group";
				break;
			case EQualifyingRule.FillWithTeam:
				if (m_Team != null)
				{
					...
					if (m_Parameter1 != 0)
					{
						text = text + " at position " + m_Parameter1;
					}
	...
	public bool SaveToTasks(StreamWriter w)
	{
		...
		if (m_When == "start")
		{
			if (m_Group != null)
			{
				text = m_Group.ParentTrophy.Id + ",start," + m_Action + "," + m_Group.Id + ",";
			}
			else if (m_Stage != null)
			{
				text = m_Stage.Trophy.Id + ",start," + m_Action + "," + m_Stage.Id + ",";
			}
			if (m_Action == "FillFromCompTable")
			{
				text = text + ((m_Trophy1 != null) ? m_Trophy1.Id.ToString() : m_Parameter1.ToString()) + "," + m_Parameter2 + "," + m_Parameter3;
			}
			else if (m_Action == "FillFromCompTableBackup")
			{
				text = text + ((m_Trophy1 != null) ? m_Trophy1.Id.ToString() : m_Parameter1.ToString()) + "," + ((m_Trophy2 != null) ? m_Trophy2.Id.ToString() : m_Parameter2.ToString()) + "," + m_Parameter3;
			}
			else if (m_Action == "FillFromCompTableBackupLeague")
			{
				text = text + ((m_Trophy1 != null) ? m_Trophy1.Id.ToString() : m_Parameter1.ToString()) + "," + ((m_League != null) ? m_League.Id.ToString() : m_Parameter2.ToString()) + "," + m_Parameter3;
			}
			else if (m_Action == "FillFromLeague")
			{
				text = text + ((m_League != null) ? m_League.Id.ToString() : m_Parameter1.ToString()) + "," + m_Parameter2 + "," + m_Parameter3;
			}
			else if (m_Action == "FillFromLeagueMaxFromCountry")
			{
				text = text + ((m_League != null) ? m_League.Id.ToString() : m_Parameter1.ToString()) + "," + m_Parameter2 + "," + m_Parameter3;
			...
			{
				text = text + m_Parameter1 + "," + m_Parameter2 + "," + m_Parameter3;
			}
			else if (m_Action == "FillWithTeam")
			{
				text = text + m_Parameter1 + "," + ((m_Team != null) ? m_Team.Id.ToString() : m_Parameter2.ToString()) + "," + m_Parameter3;
			}
			else if (m_Action == "FillFromLeagueInOrder")
			{
				text = text + ((m_League != null) ? m_League.Id.ToString() : m_Parameter1.ToString()) + "," + m_Parameter2 + "," + m_Parameter3;
		...
		else if (m_Action == "UpdateTable")
		{
			...
			{
				text = m_Trophy.Id + ",end," + m_Action + "," + m_Trophy.Id + "," + m_Group.Id + "," + m_Parameter2 + "," + m_Parameter3;
			}
			else
			{
		...
		else if (m_Action == "UpdateLeagueTable")
		{
			...
			if (m_Stage != null)
			{
				text = ((m_League == null) ? (m_Stage.Trophy.Id + ",end," + m_Action + "," + m_Stage.Id + "," + m_Parameter1 + "," + m_Parameter2 + "," + m_Parameter3) : (m_Stage.Trophy.Id + ",end," + m_Action + "," + m_Stage.Id + "," + m_League.Id + "," + m_Parameter2 + "," + m_Parameter3));
			}
			else if (m_Group != null && m_League != null)
			{
				text = m_Group.ParentTrophy.Id + ",end," + m_Action + "," + m_Group.ParentStage.Id + "," + m_League.Id + "," + m_Parameter2 + "," + m_Parameter3;
			}
			else
			{
				FifaEnvironment.UserMessages.ShowMessage(14999, "Debug Trap: Task::SaveToTasks()");
		...
		else if (m_Action == "UpdateLeagueStats")
		{
			...
			{
				text = m_Stage.Trophy.Id + ",end," + m_Action + "," + m_Stage.Id + "," + m_League.Id + "," + m_Parameter2 + "," + m_Parameter3;
			}
			else
			{
				FifaEnvironment.UserMessages.ShowMessage(14999, "Debug Trap: Task::SaveToTasks()");
