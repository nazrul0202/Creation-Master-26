...
public class Rx3File
{
	...
	public Rx3Header Rx3Header => m_Rx3Header;
	...
	public byte[] ReadSectionData(int sectionIndex)
	{
		if (sectionIndex >= m_Rx3Header.NFiles)
		{
			return null;
		}
		FileStream fileStream = new FileStream(m_FileName, FileMode.Open, FileAccess.Read);
		fileStream.Position = m_Rx3FileDescriptors[sectionIndex].Offset;
		byte[] array = new byte[m_Rx3FileDescriptors[sectionIndex].Size];
		fileStream.Read(array, 0, m_Rx3FileDescriptors[sectionIndex].Size);
		fileStream.Close();
		return array;
	}

	public bool WriteSectionData(int sectionIndex, byte[] buffer)
	{
		if (sectionIndex >= m_Rx3Header.NFiles)
		{
			return false;
		}
		if (buffer.Length > m_Rx3FileDescriptors[sectionIndex].Size)
		{
			return false;
		}
		FileStream fileStream = new FileStream(m_FileName, FileMode.Open, FileAccess.ReadWrite);
		fileStream.Position = m_Rx3FileDescriptors[sectionIndex].Offset;
		byte[] array = new byte[m_Rx3FileDescriptors[sectionIndex].Size - buffer.Length];
	...
	public bool Inject(string originalFileName)
	{
		Rx3File rx3File = new Rx3File();
		rx3File.Load(originalFileName);
		return Inject(rx3File);
	}

	public bool Inject(Rx3File originalRx3)
	{
		if (m_Rx3Header.NFiles != originalRx3.Rx3Header.NFiles)
		{
			return false;
		}
		...
		{
			bool flag = true;
			switch (m_Rx3FileDescriptors[i].Signature)
			{
			case 5798132u:
			{
				byte[] buffer = originalRx3.ReadSectionData(i);
				flag = WriteSectionData(i, buffer);
				break;
			}
			case 5798561u:
			{
	...
	public bool ReplaceBitmap(Bitmap bitmap, int imageIndex)
	{
		if (bitmap == null)
		{
			return false;
		}
		if (imageIndex < 0 || imageIndex >= m_Rx3Images.Length)
		{
			return false;
		}
		m_Rx3Images[imageIndex].SetBitmap(bitmap);
		return true;
	}
