...
public class FshSection
{
		...
		case 109:
		{
			for (int num6 = 0; num6 < bitmap.Height; num6++)
			{
				for (int num7 = 0; num7 < bitmap.Width; num7++)
				{
					...
					byte a2 = pixel5.A;
					ushort num8 = 0;
					num8 |= (ushort)((b4 & 0xF0) >> 4);
					num8 |= (ushort)(g4 & 0xF0);
					num8 |= (ushort)((r4 & 0xF0) << 4);
					num8 |= (ushort)((a2 & 0xF0) << 8);
					bw.Write(num8);
				}
			}
			break;
		...
		case 120:
		{
			for (int m = 0; m < bitmap.Height; m++)
			{
				for (int n = 0; n < bitmap.Width; n++)
				{
					...
					byte r3 = pixel3.R;
					ushort num2 = 0;
					num2 |= (ushort)((b3 & 0xF8) >> 3);
					num2 |= (ushort)((g3 & 0xFC) << 3);
					num2 |= (ushort)((r3 & 0xF8) << 8);
					bw.Write(num2);
				}
			}
			break;
	...
	public void Hash(string fileName)
	{
		...
		for (int i = 0; i < m_RawData.Length - 11; i++)
		{
			if (m_RawData[i] == 44 && m_RawData[i + 1] == 48 && m_RawData[i + 2] == 120)
			{
				i += 3;
				for (int j = 0; j < 8; j++)
				{
					m_RawData[i + j] = (byte)array[j];
				}
				break;
			}
		}
