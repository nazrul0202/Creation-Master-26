...
public class LeagueList : IdArrayList
{
	...
	public void SaveLeaguesTable(Table leaguesTable, bool resize)
	{
		if (resize)
		{
			leaguesTable.ResizeRecords(Count);
		}
		int num = 0;
		...
		try
		{
			...
			{
				League obj = (League)enumerator.Current;
				Record r = leaguesTable.Records[num];
				num++;
				obj.SaveLeague(r);
			}
		}
		...
		leaguesTable.NValidRecords = num;
		for (int i = num; i < leaguesTable.NRecords; i++)
		{
			leaguesTable.Records[i].Clean();
		}
	}
