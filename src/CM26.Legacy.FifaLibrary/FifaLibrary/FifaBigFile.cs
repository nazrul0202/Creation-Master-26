...
public class FifaBigFile : FifaFile
{
	...
	public FifaFile[] GetArchivedFiles(string searchPattern, bool useFullPath)
	{
		...
		for (int i = 0; i < m_NFiles; i++)
		{
			...
			bool flag;
			array[i] = (flag = text.Contains(searchPattern));
			if (flag)
			{
				num++;
			}
	...
	public Bitmap[] GetAllDds()
	{
		...
		Bitmap[] array = new Bitmap[num];
		num = 0;
		for (int k = 0; k < m_NFiles; k++)
		{
			if (m_Files[k].IsDds())
			{
				DdsFile ddsFile = new DdsFile(m_Files[k]);
				array[num] = ddsFile.GetBitmap();
				num++;
			}
		}
		return array;
