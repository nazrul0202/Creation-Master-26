...
public class FifaFile
{
	...
	public bool Decompress(Stream outputStream)
	{
		...
		case ECompressionMode.Chunklzma:
			return UnChunklzma(outputStream);
		}
		return true;
	}
	...
	public bool Decompress()
	{
		...
		if (num)
		{
			...
			m_IsInMemory = true;
			m_StartPosition = 0u;
			m_UncompressedSize = (int)m_ReadMemoryStream.Length;
		}
		return num;
	}
	...
	public bool SetCompressionMode(ECompressionMode compressionMode)
	{
		...
		if (m_CurrentCompression != ECompressionMode.None && compressionMode != ECompressionMode.None)
		{
			return false;
		}
		if (m_CurrentCompression == ECompressionMode.None && compressionMode != ECompressionMode.None)
	...
	private bool UnEASF(Stream outputStream)
	{
		...
		m_CompressedSize = (int)fileInfo.Length;
		fileStream.Seek(24L, SeekOrigin.Begin);
		int num = 24;
		int num2 = m_CompressedSize - 32;
		do
		{
			num += 8;
			int num3 = FifaUtil.RoundUp(num, 16) - num;
			if (num3 > 0)
			{
				binaryReader.ReadBytes(num3);
				num += num3;
			}
			num2 = FifaUtil.SwapEndian(binaryReader.ReadInt32());
			binaryReader.ReadInt32();
			num += num2;
			InflateBlock2(binaryReader.BaseStream, outputStream, num2);
		}
		while (num < m_CompressedSize);
		fileStream.Close();
		File.Delete(text);
	...
	{
		ZOutputStream outputStream2 = new ZOutputStream(outputStream);
	...
	{
		ZOutputStream outputStream2 = new ZOutputStream(outputStream, -1);
		long position = outputStream.Position;
	...
	private static void CopyStream(Stream inputStream, ZOutputStream outputStream, int size)
	{
		...
		outputStream.Write(buffer, 0, size + num);
		outputStream.finish();
		outputStream.Flush();
	}
