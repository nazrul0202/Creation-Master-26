...
public class CompobjList : IdArrayList
{
	...
	public bool LoadFromStandings(string fileName)
	{
		...
		while ((text = streamReader.ReadLine()) != null)
		{
			...
			{
				Group obj = (Group)compobj;
				if (obj != null)
				{
					Rank value = new Rank(obj, num + 1);
					obj.Ranks.Add(value);
				}
			}
		}
	...
	public bool LoadFromSchedule(string fileName)
	{
		...
		while ((text = streamReader.ReadLine()) != null)
		{
			...
			{
				Group obj = (Group)SearchId(id);
				if (obj != null)
				{
					Schedule schedule2 = new Schedule(obj, day, leg, minGames, maxGames, time);
					obj.AddSchedule(schedule2);
				}
			}
		}
		streamReader.Close();
	...
	public bool LoadFromAdvancement(string fileName)
	{
		...
		while ((text = streamReader.ReadLine()) != null)
		{
			...
			Group obj = ((idObject != null) ? ((Group)idObject) : null);
			idObject = SearchId(id3);
			Group obj2 = ((idObject != null) ? ((Group)idObject) : null);
			if (obj == null || obj2 == null)
			{
				continue;
			}
			Rank rank = (Rank)obj.Ranks.SearchId(id2);
			Rank rank2 = (Rank)obj2.Ranks.SearchId(id4);
				...
				if (array[1] == "TIER_AND_OBJECTIVES")
				{
					...
					foreach (Country country in FifaEnvironment.Countries)
					{
						if (country.NationalTeam != null)
						{
							...
							text = level + "," + id + "," + id2 + "," + num2 + "," + text2 + "," + text2 + "," + text3 + "," + text3;
							streamWriter.WriteLine(text);
						}
					}
					continue;
