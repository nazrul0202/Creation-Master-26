...
public class Nation : Compobj
{
	...
	public override bool SaveToWeather(StreamWriter w)
	{
		...
		for (int i = 0; i < 12; i++)
		{
			...
			{
				string value = base.Id + "," + (i + 1) + "," + m_ClearProb[i] + "," + m_HazyProb[i] + "," + m_CloudyProb[i] + "," + m_RainProb[i] + "," + m_ShowersProb[i] + "," + m_SnowProb[i] + "," + m_FlurriesProb[i] + "," + m_OvercastProb[i] + "," + m_FoggyProb[i] + "," + m_SunsetTime[i] + "," + m_DarkTime[i];
				w.WriteLine(value);
			}
		}
		return true;
