...
public class Group : Compobj
{
	...
	public override bool SaveToStandings(StreamWriter w)
	{
		for (int i = 1; i < m_Ranks.Count; i++)
		{
			...
			string value = base.Id + "," + (rank.Id - 1);
			w.WriteLine(value);
		}
		return true;
	}
	...
	public override bool SaveToAdvancement(StreamWriter w)
	{
		for (int i = 1; i < m_Ranks.Count; i++)
		{
			...
			{
				string value = rank.MoveFrom.Group.Id + "," + rank.MoveFrom.Id + "," + base.Id + "," + rank.Id;
				w.WriteLine(value);
			}
		}
		return true;
	...
	public override void Normalize()
	{
		...
		stage = (Stage)base.ParentObj;
		for (int l = 0; l < m_NEndTasks; l++)
		{
			if (m_EndTask[l].Action == "UpdateLeagueTable")
			{
				stage.AddTask(m_EndTask[l]);
				RemoveTask(m_EndTask[l].When, l);
				break;
			}
		}
	}
