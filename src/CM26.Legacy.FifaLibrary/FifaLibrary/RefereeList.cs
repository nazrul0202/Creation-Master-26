...
public class RefereeList : IdArrayList
{
	...
	{
		Table table = eaDbFile.GetTable("referee");
		...
		try
		{
			while (enumerator.MoveNext())
			{
				Referee obj = (Referee)enumerator.Current;
				Record r = table.Records[validRecordAndClean++];
				obj.SaveReferee(r);
			}
		}
		finally
		...
		table.SetValidRecordAndClean(validRecordAndClean);
	}
}
