...
public class FifaUtil
{
	...
	public static Random Randomizer => m_Randomizer;

	public static void RandomizeGaussianDouble(double mean1, double sigma1, out double rand1, double mean2, double sigma2, out double rand2)
	{
		double num3;
		double num4;
		double num5;
		do
		{
			double num = m_Randomizer.NextDouble();
			double num2 = m_Randomizer.NextDouble();
			num3 = 2.0 * num - 1.0;
			num4 = 2.0 * num2 - 1.0;
			num5 = num3 * num3 + num4 * num4;
		}
		while (num5 > 1.0 || num5 == 0.0);
		double num6 = Math.Sqrt(-2.0 * Math.Log(num5) / num5) * num3;
		double num7 = Math.Sqrt(-2.0 * Math.Log(num5) / num5) * num4;
		rand1 = num6 * sigma1 + mean1;
		rand2 = num7 * sigma2 + mean2;
	}

	public static int RandomizeInteger(int min, int max)
	{
		return min + m_Randomizer.Next(max - min + 1);
	}
	...
	public static string ReadString(BinaryReader r, int offset)
	{
		...
		string result = ue.GetString(r.ReadBytes(count));
	...
	public static string ReadString(BinaryReader r, long offset, int length)
	{
		long position = r.BaseStream.Position;
		r.BaseStream.Position = offset;
		string result = ue.GetString(r.ReadBytes(length));
		r.BaseStream.Position = position;
		return result;
	}
	...
	public static DateTime ConvertFromFifaDate(int fifaDate)
	{
		int num = fifaDate / 10000;
		int month = (fifaDate - num * 10000) / 100;
		int day = fifaDate % 100;
		return new DateTime(num, month, day);
	}

	public static int ConvertToFifaDate(DateTime date)
	{
		int year = date.Year;
	...
	public static int ConvertToFifaTime(DateTime date)
	{
		int hour = date.Hour;
	...
	public static int GetDeltaDays(int firstFifaDate, int secondFifaDate)
	{
		DateTime dateTime = ConvertFromFifaDate(firstFifaDate);
		DateTime value = ConvertFromFifaDate(secondFifaDate);
		return dateTime.Subtract(value).Days;
	}

	public static int AddDays(int fifaDate, int days)
	{
		DateTime date = ConvertFromFifaDate(fifaDate).AddDays(days);
		return ConvertToFifaDate(date);
	}

	public static bool EncodeSps(string inputFileName, EACodecId codecId)
	{
		...
		switch (codecId)
		{
		case EACodecId.CodecEalayer3V1:
		case EACodecId.CodecEalayer3V2Pcm:
		case EACodecId.CodecEalayer3V2Spike:
			flag = true;
			break;
		case EACodecId.CodecEaspeex:
			flag = true;
			break;
		}
		string text = "encode \"" + inputFileName + "\" ";
		if (flag)
		{
			text += "-e";
		}
		...
		ProcessStartInfo processStartInfo = new ProcessStartInfo("EASounds.exe", text);
	...
	public static bool DecodeSpsToWav(string inputFileName)
	{
		if (!File.Exists(inputFileName))
		{
			return false;
