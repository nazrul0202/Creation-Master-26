# Player Engine Data Audit — CM26

Date: 2026-07-28
Database folder: `D:\CM 26 Final\database` (read-only audit; NO external TXT/XLSX/CSV)
...
| Layer | Value | Correct? |
|-------|-------|----------|
| Physical DB `players.playerid` | `250` | ✅ |
| Native engine `firstnameid` | `8642` | ✅ correct integer |
| Native engine `lastnameid` | `4000` | ✅ correct integer |
| Native engine `commonnameid` | `0` | ✅ |
| `playernames.name` for 8642 | bytes `[C4 C4 C4 44 C4]` | ⚠ **ciphered placeholder (0xC4 fill)** |
| `playernames.name` for 4000 | bytes `[20 C4 C4 64 61 69 C4]` | ⚠ **ciphered placeholder** |
| Bridge `GetCellBytes` | returns same bytes | ✅ |
| `PlayerNameService` decode | `null` (rejected 0xC4) | ✅ honest |
| `NameResolverService` display | `Player 250` | ✅ honest fallback |
...
column and `"Player"` in the First Name column. **That bug is fixed** — the roster now uses a single
`Display Name` column and never splits the fallback. `bareNumeric=0` confirms no bare numeric name is
shown.

## 3. Full pass — 20,268 players
...
Spot-traced players across positions (GK/DEF/MID/FWD), free agents, and UTF-8 names — all return the
honest `Player {id}` fallback with `bareNumeric=0`. The decoder accepts UTF-8/CP1252 (proven on the
cipher-free `manager` table, e.g. "Rúben Filipe Marques Amorim") and correctly rejects `0xC4` cipher.

## 5. Verdict

| Layer | Verdict |
|-------|---------|
| Physical DB `playernames.name` | **ciphered** (EA placeholder) — the genuine blocker |
